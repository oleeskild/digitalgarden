...
import { userMarkdownSetup, userEleventySetup } from "./src/helpers/userSetup.js";
...
export default (function (eleventyConfig) {
    ...
    eleventyConfig.addTransform("picture", function (str) {
        ...
        for (const imageTag of parsed.querySelectorAll(".cm-s-obsidian img")) {
            ...
            if (src && src.startsWith("/") && !src.endsWith(".svg")) {
                ...
                try {
                    const meta = transformImage("./src/site" + decodeURI(imageTag.getAttribute("src")), cls.toString(), alt, ["(max-width: 480px)", "(max-width: 1024px)"]);
                    if (meta) {
    ...
    eleventyConfig.addTransform("htmlMinifier", (content, outputPath) => {
        if ((process.env.NODE_ENV === "production" || process.env.ELEVENTY_ENV === "prod") &&
            outputPath &&
